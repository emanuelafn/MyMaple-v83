...
//                    statups.add(new Pair<MapleBuffStat, Integer>(MapleBuffStat.DASH, Integer.valueOf(x))); Attempt Fix Dash
//                    statups.add(new Pair<MapleBuffStat, Integer>(MapleBuffStat.DASH2, Integer.valueOf(ret.y))); Attempt Fix Dash
                    break;
    ...
    private void applyBuff(MapleCharacter applyfrom) {
        if ((isPartyBuff() && (applyfrom.getParty() != null || isGmBuff())) || ((sourceid == Bowmaster.SHARP_EYES || sourceid == Marksman.SHARP_EYES) && (applyfrom.isSGM()))) {
            ...
            for (MapleMapObject affectedmo : affecteds) {
                ...
                if (affected != applyfrom && (isGmBuff() || applyfrom.getParty().equals(affected.getParty()) || ((sourceid == Bowmaster.SHARP_EYES || sourceid == Marksman.SHARP_EYES) && (applyfrom.isSGM())))) {
    ...
    private void applyBuffEffect(MapleCharacter applyfrom, MapleCharacter applyto, boolean primary, int oid) {
        ...
        if (localstatups.size() > 0) {
            ...
            if (isDash()) {
                ...
                //buff = MaplePacketCreator.giveMount(applyto.getMount().getId(), sourceid, localstatups);
            } else if (isCygnusFA()) {
