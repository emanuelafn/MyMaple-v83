...
if (player.getAntiKS() && antiKS < 3 && monster.getCanBelong() && player.getNumAntiKSMonsters() < 10) {
    ...
    antiKS++;
		    ...
		    if (eff.makeChanceResult()) {
                        monster.applyStatus(player, new MonsterStatusEffect(Collections.singletonMap(MonsterStatus.STUN, 1), skill, false), false, eff.getX() * 1000);
		    }
	        }
	        if (player.getBuffedValue(MapleBuffStat.COMBO_DRAIN) != null) {
		    final ISkill skill = SkillFactory.getSkill(21100005);
		    player.addHP(player.getHp() + ((totDamage * skill.getEffect(player.getSkillLevel(skill)).getX()) / 100));
		}
                ...
                if (player.getBuffedValue(MapleBuffStat.PICKPOCKET) != null && (attack.skill == 0 || attack.skill == Rogue.DOUBLE_STAB || attack.skill == Bandit.SAVAGE_BLOW || attack.skill == ChiefBandit.ASSAULTER || attack.skill == ChiefBandit.BAND_OF_THIEVES || attack.skill == Shadower.ASSASSINATE || attack.skill == Shadower.TAUNT || attack.skill == Shadower.BOOMERANG_STEP)) {
                    ...
                    totDamageToOneMonster = 1950 + Randomizer.getInstance().nextInt(5000);
