...
public final class PlayerInteractionHandler extends AbstractMaplePacketHandler {
    ...
    public enum Action {
        ...
        MERCHANT_BUY(0x22),
        TAKE_ITEM_BACK(0x25),
        MAINTENANCE_OFF(0x26),
        MERCHANT_ORGANIZE(0x27),
        CLOSE_MERCHANT(0x28),
        REQUEST_TIE(45),
        ANSWER_TIE(46),
        GIVE_UP(47),
        EXIT_AFTER_GAME(0x33),
        CANCEL_EXIT(0x34),
        READY(0x35),
        UN_READY(0x36),
        START(0x38),
    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (mode == Action.CREATE.getCode()) {
            ...
            if (c.getPlayer().getItemQuantity(ivItem.getItemId(), false) < bundles || ivItem.getFlag() == InventoryConstants.UNTRADEABLE) {
                return;
            }
            ...
            short quantity = slea.readShort();
            if (quantity < 0) {
                player.ban("Trying to buy item from merchant with quantity less than one. (Packet Editting)", true);
                return;
            }
